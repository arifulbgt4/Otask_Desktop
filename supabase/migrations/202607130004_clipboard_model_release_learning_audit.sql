create table if not exists public.clipboard_items (
  ...
  payload_ref text not null check (payload_ref ~ '^storage://clipboard/[A-Za-z0-9._/-]{1,512}$'),
...
create table if not exists public.model_packages (
  ...
  manifest_ref text not null check (manifest_ref ~ '^storage://models/[A-Za-z0-9._/-]{1,512}$'),
  weights_ref text not null check (weights_ref ~ '^storage://models/[A-Za-z0-9._/-]{1,512}$'),
  content_hash text not null check (content_hash ~ '^[a-f0-9]{64}$'),
...
create table if not exists public.model_installations (
  ...
  installed_ref text check (installed_ref is null or installed_ref ~ '^storage://models/[A-Za-z0-9._/-]{1,512}$'),
...
create table if not exists public.release_artifacts (
  ...
  download_ref text not null check (download_ref ~ '^storage://releases/[A-Za-z0-9._/-]{1,512}$'),
  content_hash text not null check (content_hash ~ '^[a-f0-9]{64}$'),
...
create table if not exists public.learning_resources (
  ...
  content_ref text not null check (content_ref ~ '^storage://learning/[A-Za-z0-9._/-]{1,512}$'),
